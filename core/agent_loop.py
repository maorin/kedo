...
class AgentLoop:
    ...
    def _gather_fix_context(self, subtask: SubTask, project_path: str) -> list[tuple[str, str]]:
        ...
        # 0) project_profile.json — build/test 命令来自它
        # human_verified=true 时不放进 fix context：防止 LLM "简化"已人工确认的
        # build command（之前 LLM 删掉 -DCMAKE_TOOLCHAIN_FILE 导致交叉编译回归）。
        # 改为只放一行 read-only 提示让 LLM 知道 profile 存在但不可改。
        ...
        if profile_file.exists() and profile_file.is_file():
            try:
                ...
                if profile and profile.human_verified:
                    results.append((
                        ".kedo/project_profile.json",
                        f"[READ-ONLY: human_verified=true, do NOT edit this file]\n"
                        f"build.command = {profile.build_command}\n"
                        f"test.strategy = {profile.test_strategy}",
                    ))
                else:
    ...
    def _build_fix_prompt(
        ...
        system = (
            ...
            "3. `.kedo/project_profile.json` is the source of truth for build/test commands. "
            "If it appears in the relevant files with `[READ-ONLY: human_verified=true]`, "
            "you MUST NOT edit it — the user has manually verified that profile. Fix the "
            "source code or build files instead. If it appears WITHOUT read-only marking, "
            "you may edit it when the build error clearly originates from a wrong command "
            "in the profile. NEVER remove -DCMAKE_TOOLCHAIN_FILE or -specs= flags from "
            "build commands — these are load-bearing cross-compilation settings.\n"
